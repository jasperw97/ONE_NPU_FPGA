...
module npu_int16 #(
    ...
    logic [14:0]                 decode_inst_reserved_r;
    ...
    logic [`SRAM_WIDTH_O-1:0]    setup_op_desc_data_r;
    ...
    // ------------------------------------------------------------
    assign load_inst_word_len = (inst_count_r + 15) >> 4;
    assign raw_inst      = inst_fetch_data[inst_pc_r[3:0]*64 +: 64];
    ...
    assign inst_fetch_addr = inst_pc_r[`SRAM_ADDR_WIDTH+3:4]; // A SRAM word has 16 instructions.
    assign fetch_desc_en   = (state == S_FETCH_DESC) && !fetch_desc_read_pending_r && next_op_needs_desc;
    assign fetch_desc_addr = next_op_desc_idx_r[`SRAM_ADDR_WIDTH-1:0];
    // ------------------------------------------------------------
        ...
        if (!rst_n) begin
            ...
            if (state == S_DECODE_INST) begin
                if (!decode_valid_r) begin
                    ...
                    decode_inst_reserved_r <= decoded_inst_reserved;
            ...
            if (state == S_FETCH_DESC) begin
                if (fetch_desc_read_pending_r) begin
                    ...
                    setup_op_desc_data_r <= fetch_desc_data;
                    setup_op_desc_valid_r <= 1'b1;
                    next_op_slot_idx_r <= fetch_desc_context_r.op_idx + 3'd1;
                    next_op_desc_idx_r <= fetch_desc_context_r.desc_idx + 25'd1;
                    fetch_desc_read_pending_r <= 1'b0;
                end else if (next_op_needs_desc) begin
