...
module ADD_Element (
    ...
    reg signed [`INT32_SIZE-1:0] stage_quantized_activation_min[0:5];
    integer i;
    always @(posedge clk or negedge rst)
    ...
    reg signed [`INT32_SIZE-1:0] stage_quantized_activation_max[0:5];
    always @(posedge clk or negedge rst)
    ...
    reg signed [`INT16_SIZE-1:0] in1_reg, in2_reg;
    always @(posedge clk or negedge rst) begin
        if (!rst) begin
            in1_reg <= 0;
            in2_reg <= 0;
    ...
    always @(posedge clk or negedge rst) begin
        if (!rst)
            ...
            out <= $signed(stage6_clamped[`INT16_SIZE-1:0]);
    end
