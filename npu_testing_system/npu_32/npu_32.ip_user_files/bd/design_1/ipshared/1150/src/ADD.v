...
(
    ...
    // 一次8筆資料
    input wire [`INT16_SIZE * LANES - 1:0] input1,
    input wire [`INT16_SIZE * LANES - 1:0] input2,
    ...
    // output signals
    output wire [`INT16_SIZE * LANES - 1:0] data_o,
    output wire valid_o
);
    // 利用 generate 拆解向量成單一元素
    wire signed [`INT16_SIZE-1:0] in1_array [0:LANES-1];
    wire signed [`INT16_SIZE-1:0] in2_array [0:LANES-1];
    
    genvar i;
    generate
        for(i = 0; i < LANES; i = i + 1) begin : input_slice
            // 使用 [高位 -: 寬度] 的語法取得每一筆 8-bit 資料
            assign in1_array[i] = $signed(input1[(i+1)*`INT16_SIZE-1 -: `INT16_SIZE]);
            assign in2_array[i] = $signed(input2[(i+1)*`INT16_SIZE-1 -: `INT16_SIZE]);
        end
    endgenerate

    // 針對每一筆資料實例化一個 ADD_element 模組
    wire signed [`INT16_SIZE-1:0] out_array [0:LANES-1];
    wire                        valid_array [0:LANES-1];

    generate
        for(i = 0; i < LANES; i = i + 1) begin : add_elements
    ...
    generate
        for(i = 0; i < LANES; i = i + 1) begin : output_assign
            assign data_o[(i+1)*`INT16_SIZE-1 -: `INT16_SIZE] = out_array[i];
        end
    endgenerate
