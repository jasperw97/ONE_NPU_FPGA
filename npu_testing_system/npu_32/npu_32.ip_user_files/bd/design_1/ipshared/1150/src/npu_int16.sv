...
module npu_int16 #(
    ...
    // Fixed DRAM regions
    // ------------------------------------------------------------
    localparam logic [`DRAM_ADDR_WIDTH-1:0] LUT_DRAM_BASE  = 32'd2_000_000;
    localparam logic [`DRAM_ADDR_WIDTH-1:0] JOB_DRAM_BASE  = 32'd6_000_000;
    localparam logic [`DRAM_ADDR_WIDTH-1:0] INST_DRAM_BASE = 32'd5_000_000;
    localparam logic [`DRAM_ADDR_WIDTH-1:0] DESC_DRAM_BASE = 32'd7_000_000;
    // Result stores currently use the fixed DRAM output region used by the TB.
    localparam logic [`DRAM_ADDR_WIDTH-1:0] RESULT_DRAM_BASE = 32'd3_000_000;
    localparam logic [`DRAM_ADDR_WIDTH-1:0] TENSOR_DMA_TABLE_DRAM_BASE = 32'd7_100_000;
    // ------------------------------------------------------------
    // Job header layout
    // ------------------------------------------------------------
    ...
    localparam int DESC_WORD_IDX_WIDTH =
        (DESC_WORDS_PER_PACKET <= 1) ? 1 : $clog2(DESC_WORDS_PER_PACKET);
    ...
    logic [5:0] ew_src1_broadcast_step;

    // LUT for modulo during setup
    logic [5:0] init_lane_idx_rom [0:LANES-1][0:64];
    logic [5:0] step_size_rom [0:64];

    initial begin
        for (int c = 0; c <= 64; c = c + 1) begin
            // Avoid division by zero in simulation, fallback to 0
            step_size_rom[c] = (c == 0) ? 0 : (LANES % c);
            for (int b = 0; b < LANES; b = b + 1) begin
                init_lane_idx_rom[b][c] = (c == 0) ? 0 : (b % c);
            end
        end
    end
    ...
    function automatic logic [6:0] ew_broadcast_limit_count(
        input logic [`INT32_SIZE-1:0] count
    );
        begin
            if (count <= 1)
                ew_broadcast_limit_count = 7'd1;
            else if (count >= 64)
                ew_broadcast_limit_count = 7'd64;
            else
                ew_broadcast_limit_count = count[6:0];
        end
    endfunction

    function automatic logic [5:0] ew_broadcast_mod_u7(
        ...
        begin
            if (count <= 1) begin
                ew_broadcast_mod_u7 = 6'd0;
            end else if (count >= 7'd64) begin
                ew_broadcast_mod_u7 = value[5:0];
            end else begin
                rem = value[6:0];
                for (int shift_i = 6; shift_i >= 0; shift_i = shift_i - 1) begin
                    step = {7'd0, count} << shift_i;
                    if ((step <= 14'd127) && ({7'd0, rem} >= step)) begin
                        rem = rem - step[6:0];
                    end
                end
                ew_broadcast_mod_u7 = rem[5:0];
            end
        end
    endfunction
    // ------------------------------------------------------------
    ...
    // Frontend preload write pointer; payloads append per tensor SRAM bank.
    logic [`SRAM_ADDR_WIDTH-1:0] tensor_load_wr_ptr [0:7];
    // ------------------------------------------------------------
    ...
    logic [`INT32_SIZE-1:0]      cfg_ew_src1_broadcast_count;
    ...
    logic [`INT16_SIZE*LANES-1:0]          ew_src1_broadcast_data;
    logic [5:0]                            ew_src1_broadcast_lane_idx [0:LANES-1];
    logic [5:0]                            ew_src1_broadcast_base_idx;
    logic [6:0]                            ew_src1_broadcast_count_q;
    logic [CFG_N_WIDTH:0]                  ew_src1_broadcast_word_in_row;
    logic [CFG_N_WIDTH:0]                  ew_src1_broadcast_words_per_row;
    ...
    // Pre-clamp the 32-bit config value to a 7-bit index for the ROM
    wire [6:0] clamped_broadcast_count = ew_broadcast_limit_count(cfg_ew_src1_broadcast_count);
        ...
        unique case (state)
            ...
            S_LOAD_LUT: begin
                ...
                if ((lut_phase_r == LUT_WAIT_BOTH) && exp_lut_ready && rcp_lut_ready) begin
                    state_n = (desc_word_len_r == '0) ? S_LOAD_TENSOR : S_LOAD_DESC;
                end
            end
            S_LOAD_DESC: begin
                if ((desc_word_len_r == '0) || (lsu_cmd_issued_r && lsu_complete)) begin
                    state_n = S_LOAD_TENSOR;
                end
            end
            S_LOAD_TENSOR: begin
                if ((tensor_dma_count_r == '0) || !tensor_entry_idx_valid) begin
            ...
            S_DECODE_INST: begin
                if (decode_valid_r) state_n = S_NEXT_OP;
            end
            S_NEXT_OP: begin
            ...
            S_SETUP_OP: begin
                state_n = setup_op_issue_valid ? S_ISSUE_OP : S_NEXT_OP;
            end
            S_ISSUE_OP: begin
                state_n = S_WAIT_OP;
            end
    ...
    always_comb begin
        ...
        end else if (tensor_dma_count_r == '0) begin
            tensor_phase_n = TENSOR_ENTRY;
        end else begin
    ...
    assign tensor_entry_is_last             = ((tensor_entry_idx_r + 1'b1) >= tensor_dma_count_r);
    // ------------------------------------------------------------
    ...
    assign lsu_valid = ((state == S_LOAD_JOB) ||
                       ...
                       !lsu_cmd_issued_r;
    assign lsu_write = (state == S_STORE_RESULT);
    ...
    assign lsu_dram_base = (state == S_LOAD_JOB)        ? JOB_DRAM_BASE
                         : (state == S_LOAD_INST)       ? INST_DRAM_BASE
                         : (state == S_STORE_RESULT)    ? RESULT_DRAM_BASE
                         : lsu_load_tensor_entry        ? (TENSOR_DMA_TABLE_DRAM_BASE +
                                                           {{(`DRAM_ADDR_WIDTH-`SRAM_ADDR_WIDTH){1'b0}}, tensor_entry_idx_r})
                         : lsu_load_tensor_payload      ? tensor_entry_dram_base_r
                         : (state == S_LOAD_DESC)       ? DESC_DRAM_BASE
                         : (lut_phase_r == LUT_DMA_EXP) ? EXP_LUT_DRAM_BASE
                         :                                RCP_LUT_DRAM_BASE;

    assign lsu_sram_base = (state == S_STORE_RESULT)    ? store_result_sram_base_r
                         : lsu_load_tensor_payload      ? tensor_load_wr_ptr[tensor_entry_bank_r[2:0]]
    ...
    assign cfg_ew_src1_broadcast       = (setup_r.op_context.b_cast != 2'd0);
    assign cfg_ew_src1_broadcast_count = setup_r.desc.broadcast_count;

    assign ctrl_busy = (state != S_IDLE) && (state != S_DONE);
    ...
    assign ew_src1_addr = cfg_ew_src1_broadcast ? '0 : load_sram_addr;

    assign ew_src1_data_selected = cfg_ew_src1_broadcast ? ew_src1_broadcast_data
                                                           : tensor_src1_data;

    // src0 priority: LSU STORE > descriptor-selected EW/SA src0 bank.
    // This lets op0 EW read input1 from its own bank.
    assign tensor_src0_bank  = lsu_store_sram_ren ? lsu_store_sram_bank
                                                  : cfg_src0_bank;
    assign tensor_src0_ren   = lsu_store_sram_ren | load_sram_en | sa_sram_req_valid;
    assign tensor_src0_addr  = lsu_store_sram_ren ? lsu_store_sram_addr
                            : load_sram_en        ? load_sram_addr
                            :                       sa_data_addr;
    assign tensor_src1_bank  = cfg_src1_bank;
    assign tensor_src1_ren   = ew_src1_ren | sa_sram_req_valid;
    assign tensor_src1_addr  = ew_src1_ren ? ew_src1_addr
                                           : sa_weight_addr;

    //Pipeline this part for implementation timing
    ...
    // assign tensor_dst_waddr  = tensor_compute_we ? (ew_we ? ew_waddr : sa_waddr)
    //                                              : lsu_bank_waddr;
    // assign tensor_dst_wdata  = tensor_compute_we ? (ew_we ? ew_wdata : sa_out_data_i16)
    //                                              : lsu_bank_wdata;

    logic [2:0]                            tensor_dst_bank_comb;
    logic                                  tensor_dst_we_comb;
    logic [`SRAM_ADDR_WIDTH-1:0]           tensor_dst_waddr_comb;
    logic [`INT16_SIZE*LANES-1:0]          tensor_dst_wdata_comb;
    logic                                  tensor_compute_we_comb;
    always_comb begin
        tensor_compute_we_comb = ew_we | sa_out_valid_i16;
        tensor_dst_bank_comb   = tensor_compute_we_comb ? cfg_dst_bank : lsu_bank_wbank;
        tensor_dst_we_comb     = tensor_compute_we_comb | lsu_bank_we;
        tensor_dst_waddr_comb  = tensor_compute_we_comb ? (ew_we ? ew_waddr : sa_waddr) : lsu_bank_waddr;
        tensor_dst_wdata_comb  = tensor_compute_we_comb ? (ew_we ? ew_wdata : sa_out_data_i16) : lsu_bank_wdata;
    end


    // ============================================================
    // PIPELINE REGISTER STAGING
    // ============================================================
    always_ff @(posedge clk) begin
        if (!rst_n) begin
            ...
            // Capture the static data payload
            tensor_dst_wdata <= tensor_dst_wdata_comb;

            tensor_dst_bank <= tensor_dst_bank_comb;
            

            tensor_dst_we <= tensor_dst_we_comb;
            

            tensor_dst_waddr <= tensor_dst_waddr_comb;
    ...
    assign load_sram_rdata    = tensor_src0_data;

    // ============================================================
    // Combinational Datapath
    // ============================================================
    // ------------------------------------------------------------
    // Element-wise broadcast lane remap
    // ------------------------------------------------------------
    always_comb begin
        ew_src1_broadcast_data = '0;
        for (int bi = 0; bi < LANES; bi = bi + 1) begin
            ew_src1_broadcast_data[((bi+1)*`INT16_SIZE-1) -: `INT16_SIZE] =
                tensor_src1_data[((ew_src1_broadcast_lane_idx[bi]+1)*`INT16_SIZE-1) -: `INT16_SIZE];
        end
    end
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            tensor_entry_word_len_r <= '0;
            for (int ti = 0; ti < 8; ti = ti + 1) begin
                tensor_load_wr_ptr[ti] <= '0;
            end
            decode_valid_r <= 1'b0;
            ...
            if (state == S_IDLE && state_n == S_LOAD_JOB) begin
                ...
                lsu_cmd_issued_r <= 1'b0;
                         ...
                         ((state_n == S_LOAD_DESC) || (state_n == S_LOAD_TENSOR))) begin
                lsu_cmd_issued_r <= 1'b0;
            end else if (state == S_LOAD_DESC && state_n == S_LOAD_TENSOR) begin
                lsu_cmd_issued_r <= 1'b0;
            ...
            if (tensor_payload_done) begin
                // Append-only preload policy for now; no lifetime reuse here yet.
                tensor_load_wr_ptr[tensor_entry_bank_r[2:0]] <=
                    tensor_load_wr_ptr[tensor_entry_bank_r[2:0]] + tensor_entry_word_len_r;
                tensor_entry_idx_r <= tensor_entry_idx_r + 1'b1;
            end
    ...
    // Precompute broadcast lane indices for the SRAM data that returns one cycle
    // after each EW read request. This keeps descriptor broadcast_count out of
    // the same-cycle data path into MUL_IN2_REQUANT.
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ew_src1_broadcast_base_idx <= 6'd0;
            ew_src1_broadcast_count_q <= 7'd1;
            ew_src1_broadcast_word_in_row <= '0;
            ew_src1_broadcast_step <= 6'd0;
            ew_src1_broadcast_words_per_row <= {{CFG_N_WIDTH{1'b0}}, 1'b1};
            for (int bi = 0; bi < LANES; bi = bi + 1) begin
                ew_src1_broadcast_lane_idx[bi] <= 6'd0;
            end
        end else if (issue_op_valid && op_is_ew(issue_op_code)) begin
            ew_src1_broadcast_base_idx <= 6'd0;
            ew_src1_broadcast_count_q <= ew_broadcast_limit_count(cfg_ew_src1_broadcast_count);
            ew_src1_broadcast_word_in_row <= '0;
            // ew_src1_broadcast_step <= ew_broadcast_mod_u7(6'd64, ew_broadcast_limit_count(cfg_ew_src1_broadcast_count));
            ew_src1_broadcast_step <= step_size_rom[clamped_broadcast_count];
            // ew_src1_broadcast_words_per_row <= (((cfg_n + (LANES-1)) / LANES) == 0) ?
            //                                     {{CFG_N_WIDTH{1'b0}}, 1'b1} :
            //                                     ((cfg_n + (LANES-1)) / LANES);
            for (int bi = 0; bi < LANES; bi = bi + 1) begin
                // ew_src1_broadcast_lane_idx[bi] <= 6'd0;
                // ew_src1_broadcast_lane_idx[bi] <= ew_broadcast_mod_u7(bi, ew_broadcast_limit_count(cfg_ew_src1_broadcast_count));;
                ew_src1_broadcast_lane_idx[bi] <= init_lane_idx_rom[bi][clamped_broadcast_count];
            end
        end else if (load_sram_en && cfg_ew_src1_broadcast) begin
            for (int bi = 0; bi < LANES; bi = bi + 1) begin
                // ew_src1_broadcast_lane_idx[bi] <=
                //     ew_broadcast_mod_u7(ew_src1_broadcast_base_idx + bi, ew_src1_broadcast_count_q);
                if ((ew_src1_broadcast_lane_idx[bi] + ew_src1_broadcast_step) >= ew_src1_broadcast_count_q) begin
                    ew_src1_broadcast_lane_idx[bi] <= (ew_src1_broadcast_lane_idx[bi] + ew_src1_broadcast_step) - ew_src1_broadcast_count_q;
                end else begin
                    ew_src1_broadcast_lane_idx[bi] <= (ew_src1_broadcast_lane_idx[bi] + ew_src1_broadcast_step);
        end
            end

            if ((ew_src1_broadcast_word_in_row + 1'b1) >= ew_src1_broadcast_words_per_row) begin
                ew_src1_broadcast_word_in_row <= '0;
                // ew_src1_broadcast_base_idx <= 6'd0;
            end else begin
                ew_src1_broadcast_word_in_row <= ew_src1_broadcast_word_in_row + 1'b1;
                // ew_src1_broadcast_base_idx <=
                //     ew_broadcast_mod_u7(ew_src1_broadcast_base_idx + LANES, ew_src1_broadcast_count_q);
            end
        end
    end
    ...
    sram_unit #(
        ...
        //Replicated signals to solve high fanout
        // .dst_bank_a(tensor_dst_bank_A),
        // .dst_bank_b(tensor_dst_bank_B),
        // .dst_bank_c(tensor_dst_bank_C),
        // .dst_bank_d(tensor_dst_bank_D),

        // .dst_we_a(tensor_dst_we_A),
        // .dst_we_b(tensor_dst_we_B),
        // .dst_we_c(tensor_dst_we_C),
        // .dst_we_d(tensor_dst_we_D),

        // .dst_waddr_a(tensor_dst_waddr_A),
        // .dst_waddr_b(tensor_dst_waddr_B),
        // .dst_waddr_c(tensor_dst_waddr_C),
        // .dst_waddr_d(tensor_dst_waddr_D),
