...
module npu_int16_burst #(
    ...
    input  logic [`SRAM_WIDTH_O-1:0]      dram_r_data,
    ...
    output logic [`SRAM_WIDTH_O-1:0]      dram_w_data,
    ...
    logic [`SRAM_WIDTH_O-1:0] dram_w_data_lsu;
    logic [7:0] dram_aw_len_lsu;
            ...
            unique case (tensor_phase_r)
                TENSOR_ENTRY: begin
                    if (tensor_entry_done) begin
                        ...
                        tensor_phase_n = ((tensor_entry_word_len_r == '0) ||
                                          !tensor_entry_bank_valid) ? TENSOR_ENTRY
                                       : TENSOR_PAYLOAD;
                    end
                end
    ...
    // Muxing write data going out to dram, needs to be fixed with new axi burst
    assign dram_w_data = (state == S_STORE_PROFILE) ? { {(`SRAM_WIDTH_O - 128){1'b0}}, total_cycles_r, ew_cycles_r, sa_cycles_r, dram_cycles_r } : dram_w_data_lsu;
    assign dram_aw_len = (state == S_STORE_PROFILE) ? 8'd0 : dram_aw_len_lsu;
    ...
    LSU_axi_burst #(
        ...
        .dram_aw_len    (dram_aw_len_lsu),

        // AXI Write Data Channel
        .dram_w_valid   (dram_w_valid),
        .dram_w_ready   (dram_w_ready),
        .dram_w_data    (dram_w_data_lsu),
