...
module npu_int16 #(
        ...
        if (!rst_n) begin
            ...
            if (state == S_FETCH_DESC) begin
                if (fetch_desc_read_pending_r) begin
                    ...
                    fetch_desc_packet_r[fetch_desc_word_idx_r*`SRAM_WIDTH_O +: `SRAM_WIDTH_O] <= fetch_desc_data;
                    fetch_desc_read_pending_r <= 1'b0;
                    if (fetch_desc_last_word) begin
                        setup_op_context_r <= fetch_desc_context_r;
                        setup_op_desc_data_r <= fetch_desc_packet_complete[`DESC_PACKET_BITS-1:0];
                        setup_op_desc_valid_r <= 1'b1;
                        next_op_slot_idx_r <= fetch_desc_context_r.op_idx + 3'd1;
                        next_op_desc_idx_r <= fetch_desc_context_r.desc_idx + DESC_WORDS_PER_PACKET;
                        fetch_desc_word_idx_r <= '0;
                    end else begin
    ...
    // ------------------------------------------------------------
    SystolicArray_OS_INT8_REQUANT_SRAM u_sa (
        .clk(clk),
        .rst_n(rst_n),
